...
def verificar_acesso_rh():
    ...
    if setor not in ("cpd", "administrativo", "rh"):
        ...
        return render_template("401.html", message="Acesso restrito ao setor administrativo")
    return None
...
def listar_funcionarios():
    try:
        ...
        for func in funcionarios:
            ...
            func["total_logins"] = len(login_times) if isinstance(login_times, list) else 0
        ...
        return jsonify({"success": True, "funcionarios": funcionarios, "total": len(funcionarios)})
...
def criar_funcionario():
    try:
        ...
        if not nome_completo or not username or not turno:
            return jsonify({"status": "error", "message": "Nome completo, username e turno são obrigatórios"})

        if cpppac.usuarios.find_one({"username": username}):
            return jsonify({"status": "error", "message": f"Username '{username}' já existe"})
        ...
        if resultado.inserted_id:
            return jsonify({"status": "success", "message": "Funcionário criado com sucesso", "username": username})
...
def editar_funcionario(username):
    try:
        ...
        resultado = cpppac.usuarios.update_one(
            {"username": username},
            {"$set": {"nome_completo": nome, "turno": turno, "lvl": lvl,
                      "editado_em": datetime.datetime.now(), "editado_por": username_editor}},
        )
...
def excluir_funcionario(username):
    try:
        ...
        if resultado.deleted_count > 0:
            return jsonify({"message": f"Funcionário {nome_funcionario} excluído com sucesso"}), 200
