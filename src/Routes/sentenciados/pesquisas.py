...
def lista_pesquisas():
    ...
    if data_pesquisa:
        try:
            ...
            documentos = list(cpppac.sentenciados.find(
                {"visitas": {"$elemMatch": {"$gte": data_inicio, "$lte": data_fim}}},
                {"_id": 0},
            ).sort("nome", 1))

            for doc in documentos:
                visitas_data = [v for v in doc.get("visitas", []) if data_inicio <= v <= data_fim]
                doc["data_visita"] = visitas_data[-1].strftime("%d/%m/%Y %H:%M") if visitas_data else ""
            ...
            resumo = {"data_atual": data_obj.strftime("%d/%m/%Y"), "matriculas": len(documentos),
                      "garrafas": 0, "homens": 0, "mulheres": 0, "criancas": 0, "total_visitantes": 0}
            return render_template("lista.html", documentos=documentos, resumo=resumo)

        except ValueError:
            resumo = {"data_atual": "Data inválida", "matriculas": 0, "garrafas": 0,
                      "homens": 0, "mulheres": 0, "criancas": 0, "total_visitantes": 0}
            return render_template("lista.html", documentos=[], resumo=resumo)

    resumo = {"data_atual": "Selecione uma data", "matriculas": 0, "garrafas": 0,
              "homens": 0, "mulheres": 0, "criancas": 0, "total_visitantes": 0}
    return render_template("lista.html", documentos=[], resumo=resumo)
