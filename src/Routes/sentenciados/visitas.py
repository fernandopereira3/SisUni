import pandas as pd
from flask import render_template, request, flash, redirect, url_for, make_response, Blueprint, session
...
df_visitas = pd.DataFrame(columns=["matricula", "nome", "visitante", "parentesco", "data_visita"])
...
def jumbo():
    ...
    try:
        ...
        if not df_visitas.empty:
            df_visitas = df_visitas[pd.to_datetime(df_visitas["data_visita"]).dt.date == today]
        ...
        if request.method == "POST":
            if "pesquisar" in request.form:
                ...
                existing_mongo = cpppac.visita.find_one({"matricula": matricula, "visitante": visitante_n})
                nova_visita = {"matricula": matricula, "nome": nome_n, "visitante": visitante_n,
                               "parentesco": parentesco_n, "data_visita": pd.Timestamp.now()}

                global df_visitas
                df_visitas = pd.concat([df_visitas, pd.DataFrame([nova_visita])], ignore_index=True)

                if existing_mongo:
                    flash("Visita já existe no Banco de dados", "info")
                else:
                    cpppac.visita.insert_one(nova_visita)
                    flash("Visita registrada com sucesso no histórico permanente", "success")
        ...
        return render_template("jumbo.html", form=form, visitas=df_visitas.to_dict("records"))
...
def download_visitas_pdf():
    ...
    try:
        ...
        for _, row in df_visitas.iterrows():
            table_data.append([row["matricula"], row["nome"], row["visitante"], row["parentesco"]])
        ...
        response.headers["Content-Disposition"] = f"attachment; filename=lista_visitas_{hoje}.pdf"
        return response
    except Exception as e:
